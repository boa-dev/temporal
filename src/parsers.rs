...
pub(crate) fn parse_year_month(source: &[u8]) -> TemporalResult<IxdtfParseRecord> {
    ...
    if let Ok(ym) = ym_record {
        if ym.offset == Some(UtcOffsetRecordOrZ::Z) {
            return Err(TemporalError::range()
                .with_message("UTC designator is not valid for YearMonth parsing."));
        }

        // Only validate ECMAScript Temporal specific requirements
        if let Some(date_record) = ym.date {
            validate_date_record_impl(date_record).map_err(|e| e.into_temporal_error())?;
        }
...
pub(crate) fn parse_time(source: &[u8]) -> TemporalResult<TimeRecord> {
    ...
    let time_err = match time_record {
        Ok(time) => {
            ...
            // Validate ECMAScript Temporal specific requirements
            if let Some(time_record) = time.time {
                validate_time_record_impl(time_record).map_err(|e| e.into_temporal_error())?;
                return Ok(time_record);
            }
